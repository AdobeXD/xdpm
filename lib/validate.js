...
function validate(manifest, { root, id } = {}) {
    ...
    if (!manifest.id) {
        ...
        if (manifest.id.length !== 8) {
            errors.push(
                `F1002: Manifest 'id' character count is incorrect. Count is ${manifest.id.length}, expected 8.`
            );
        }
    }
    ...
    if (!manifest.name) {
        ...
        if (
            ...
            errors.push(
                `F1011: Manifest name is not an appropriate length (expected ${PLUGIN_NAME_MIN_LEN} - ${PLUGIN_NAME_MAX_LEN} chars, saw ${manifest.name.length} chars).`
            );
        }
    }
    ...
    if (!manifest.version) {
        ...
        if (!PLUGIN_VERSION_REGEX.test(manifest.version)) {
            errors.push(
                `F1031: Version format is incorrect. Saw ${manifest.version}, expected ${PLUGIN_MIN_VERSION} - ${PLUGIN_MAX_VERSION}.`
            );
        }
    }
    ...
    if (!manifest.host) {
        ...
        if (!manifest.host.minVersion) {
            ...
            if (!HOST_VERSION_REGEX.test(manifest.host.minVersion)) {
                errors.push(
                    `F1024: Host minimum version format is incorrect. Saw ${manifest.host.minVersion}, expected ${HOST_MIN_VERSION} - ${HOST_MAX_VERSION}.`
                );
            }
        }
        ...
        if (manifest.host.maxVersion) {
            if (!HOST_VERSION_REGEX.test(manifest.host.minVersion)) {
                errors.push(
                    `F1025: Host maximum version format is incorrect. Saw ${manifest.host.maxVersion}, expected ${HOST_MIN_VERSION} - ${HOST_MAX_VERSION}.`
                );
            }
        }
    }
    ...
    if (!manifest.icons) {
        ...
        if (
            ...
            manifest.icons.length !== ICONS_LEN
        ) {
            errors.push(
    ...
    if (!manifest.description) {
        ...
        if (
            ...
            errors.push(
                `F1051: Manifest description is not an appropriate length (expected ${PLUGIN_DESC_MIN_LEN} - ${PLUGIN_DESC_MAX_LEN} chars, saw ${manifest.description.length} chars).`
            );
        }
    }
    ...
    if (!manifest.summary) {
        ...
        if (
            ...
            errors.push(
                `F1051: Manifest summary is not an appropriate length (expected ${PLUGIN_SUMMARY_MIN_LEN} - ${PLUGIN_SUMMARY_MAX_LEN} chars, saw ${manifest.summary.length} chars).`
            );
        }
    }
    ...
    if (manifest.keywords) {
        if (!Array.isArray(manifest.keywords)) {
            ...
            const concatKeywords = manifest.keywords.reduce(
                (initVal, keyword) =>
                    keyword.length >= KEYWORD_MIN_LEN
                        ? initVal + keyword
                        : errors.push(
                              `W2011: Keywords should be at least ${KEYWORD_MIN_LEN} chars. Found keyword ${keyword} with ${keyword.length} chars.`
                          )
            );
            ...
            if (concatKeywords.length > KEYWORD_CONCAT_MAX_LEN) {
                errors.push(
                    `W2012: Concatenated length of all keywords should be no more than ${KEYWORD_CONCAT_MAX_LEN} chars. Found ${concatKeywords.length} chars.`
                );
            }
        }
    }
    ...
    if (manifest.releaseNotes) {
        if (
            ...
            errors.push(
                `W2020: Release notes are not an appropriate length (expected ${RELEASE_NOTES_MIN_LEN} - ${RELEASE_NOTES_MAX_LEN} chars, saw ${manifest.releaseNotes.length} chars).`
            );
        }
    }
    ...
    if (!manifest.author) {
        ...
        if (
            ...
            errors.push(
                `F1081: Author is not an appropriate length (expected ${AUTHOR_MIN_LEN} - ${AUTHOR_MAX_LEN} chars, saw ${manifest.author.length} chars).`
            );
        }
    }
